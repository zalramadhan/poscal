...
export default function StockOutPage() {
  const [items, setItems] = React.useState([{ productId: '', quantity: '', reason: '' }])
    ...
    <div>
      ...
      <Card className="max-w-3xl">
        ...
        <CardContent className="space-y-6">
          <Select label="Warehouse" placeholder="Select source warehouse" options={[
          ...
          <div className="space-y-3">
            ...
            {items.map((item, index) => (
              <div key={index} className="flex items-end gap-3 p-3 rounded-lg bg-muted/30">
                ...
                <div className="w-24">
                  <Input label="Qty" type="number" placeholder="0" value={item.quantity} onChange={(e) => updateItem(index, 'quantity', e.target.value)} />
                </div>
                <div className="flex-1">
                  <Input label="Reason" placeholder="Optional" value={item.reason} onChange={(e) => updateItem(index, 'reason', e.target.value)} />
                </div>
                {items.length > 1 && (
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            <Button><Save className="h-4 w-4 mr-2" />Save Stock Out</Button>
          </div>
        </CardContent>
      </Card>
    </div>
